...
namespace ShareDeployed.Proxy
{
	...
	public class DefaultProxyInvocationHandler : IProxyInvocationHandler
	{
		private readonly object _target;

		public DefaultProxyInvocationHandler(object target)
		{
			_target = target;
		}
		...
		public object Invoke(object proxy, MethodInfo method, object[] parameters)
		{
			...
			if (true)//SecurityManager.IsMethodInRole(userRole, method.Name))
			{
				// The actual method is invoked
				retVal = method.Invoke(_target, parameters);
			}
			else
			{
				throw new InvalidOperationException("Invalid permission to invoke " + method.Name);
			}
			return retVal;
		}
	}
			...
			if (interfaceMethods != null)
			{
				for (int i = 0; i < interfaceMethods.Length; i++)
				{
					...
					MethodBuilder methodBuilder = typeBuilder.DefineMethod(methodInfo.Name,
								MethodAttributes.Public | MethodAttributes.Virtual, CallingConventions.Standard, methodInfo.ReturnType, methodParameters);
